...
public class PIM_AddEmployee {
	...
	public void userNavigatesToPIMEmployeeListTag()  {
		...
		// Below code makes sure that the navigation to PIM is successful before moving to next step
	...
	public void userClicksOnAddButton() {
		...
		s.assertEquals(redUrl, "https://opensource-demo.orangehrmlive.com/index.php/pim/addEmployee", "User redirected correctly");
		log.info("User redirected to = " + redUrl );
	...
	public void userClicksOnSaveButtonWithoutEnteringFirstNameOrLastName()  {
		...
		w.until(ExpectedConditions.presenceOfElementLocated(By.xpath("//span[@class='validation-error' and contains(@for, 'lastName')]")));
		w.until(ExpectedConditions.presenceOfElementLocated(By.xpath("//span[@class='validation-error' and contains(@for, 'firstName')]")));
		
		String errorMsgFN = cdriver.findElement(By.xpath("//span[@class='validation-error' and contains(@for, 'firstName')]")).getText();
		String errorMsgLN = cdriver.findElement(By.xpath("//span[@class='validation-error' and contains(@for, 'lastName')]")).getText();
		
		WebElement t = cdriver.findElement(By.xpath("//span[@class='validation-error' and contains(@for, 'firstName')]"));
		String stf = t.getCssValue("color");
		String c = Color.fromString(stf).asHex();
		
		WebElement t1 = cdriver.findElement(By.xpath("//span[@class='validation-error' and contains(@for, 'lastName')]"));
	...
	public void verifyIfNewEmployeeIsAdded() throws InterruptedException {
		...
		String empFirstName = cdriver.findElement(By.xpath("//*[@id=\"resultTable\"]//tbody//tr[1]//td[3]//a")).getAttribute("a");
		String empLastName = cdriver.findElement(By.xpath("//*[@id=\"resultTable\"]//tbody//tr[1]//td[4]//a")).getAttribute("a");
